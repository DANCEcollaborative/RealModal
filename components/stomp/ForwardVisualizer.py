from components.stomp.BaseMessenger import ImageMessenger
...
class ForwardVisualizer(ImageMessenger):
    ...
    def build_addr_from_config(self, config=None):
        ...
        if "addr_in" in config:
            addr_in = config.addr_in
        else:
            addr_in = (config.server_ip, config.port.downstream)
        if "addr_out" in config:
            addr_out = config.addr_out
        else:
            addr_out = (config.server_ip, config.port.upstream)
    ...
    def start(self):
        ...
        while self.running:
            for key in self.config.get("CameraToDisplay", []):
                if not self.running:
                    break
    ...
    def process_image(self, img):
        if self.running:
            if 'camera_id' not in self.property:
                return
            pair = self.decode_msg(img, self.config.get("PSIImageFormat", "jpg"))
            if pair is None:
                return
            else:
